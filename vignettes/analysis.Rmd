...
For initial pots to visualize the data these values are not removed. 
...
ci <- map(corr_matrix, ~ .x %>%
  ...
  mutate(
    ...
    x = rep(c(0.4, 0.41, 0.4), each = 3),
    y = rep(c(0.01, 0.04, 0.07), each = 3)
  )
...
(gg_ab <- ggarrange(gg_ab1, gg_ab2, ncol = 2) %>%
  ...
  # bottom = text_grob("Pairwise comparison of Models; blue line shows the Loess smother; the red line shows a theroratical perfect
...
ggsave("gg_corr.png", gg_corr, width = 11, height = 6, bg = "white", dpi = 300)
