...
data_altman <- data_above10 %>% 
    pivot_wider(names_from = type) %>% 
    select(datetime, measurement, baseline, calibration) %>% 
    mutate(mean_baseline = (measurement + baseline) / 2,
...
matrix_baseline$byClass %>%
  as_tibble() %>%
  mutate(class = rownames(matrix_baseline$byClass),
  type = "baseline") %>%
  filter(class != "Class: nothing") %>%
  bind_rows(
    matrix_calibration$byClass %>%
  as_tibble() %>%
  mutate(class = rownames(matrix_calibration$byClass),
  type = "calibration") %>%
  filter(class != "Class: nothing")
  ) %>%
...
npar_contr <- map(species$taxon[-3], ~
  ...
  npar_contr[[1]]$Analysis %>%
    bind_rows(npar_contr[[2]]$Analysis,
